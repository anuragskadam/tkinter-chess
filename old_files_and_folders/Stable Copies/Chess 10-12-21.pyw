...
class Boxes:
    ...
    def pieceTeleporter(self, event, victimCode=1000):
        ...
        if PIECE_CLICK_VAR < 1000 and boxesList.index(self) in final_destination_giver(
            ...
            boxesList[pieceCodeToPositionConverterMainBoard(
                PIECE_CLICK_VAR)].piece_contained = 1000
    ...
    def clickFunc(self, event):
        ...
        if CHECK_MATE_STATUS == 0:
            if which_side_move() == self.colour:
                ...
                SIDE_POINTS[int(self.colour)
                            ] -= PIECE_WEIGHT_LIST[self.typeCode]
...
class King(Pieces):
    def __init__(self, widget, typeCode, pieceCode, colour, column, row, lifeState, movesPlayedByPiece,
        ...
        super().__init__(widget, typeCode, pieceCode, colour,
                         column, row, lifeState, movesPlayedByPiece, art)
        ...
        if piecesList[pieceCodeInput].typeCode in rookBishopQueenTypeCodeList:
            ...
            for v in unitVectorList[rookBishopQueenTypeCodeList.index(piecesList[pieceCodeInput].typeCode)]:
                for i in range(7):
                    ...
                    position = 8 * (column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + (1 + i) * v[
                        0]) + row_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + (
                        1 + i) * v[1]
                    if column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + (1 + i) * v[0] in range(
                            8) and row_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + (
                    ...
                    elif column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + (1 + i) * v[0] in range(
                        8) and row_of_piece(
            ...
            for a in positionCheckerIndexList[0]:
                for b in positionCheckerIndexList[1]:
                    positionList = [[column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + a,
                                     row_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + b],
                                    [column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + b,
                                     row_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT) + a]]
            ...
            pawnLongitudnalCoordinate = [row_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT),
                                         column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT)][
                HORIZONTAL_VERTICAL_ARRANGEMENT_VAR]
            pawnLateralCoordinate = [column_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT),
                                     row_of_piece(pieceCodeInput, ENTIRE_BOARD_MATRIX_INPUT)][
                HORIZONTAL_VERTICAL_ARRANGEMENT_VAR]
...
def final_destination_giver(piece_code_input, INPUT_BOARD_MATRIX=ENTIRE_BOARD_MATRIX, INPUT_PIECES_ALIVE=PIECES_ALIVE,
                            INPUT_MOVES_PLAYED=MOVES_PLAYED):
    ...
    initially_allowed_boxes = simple_possible_destination_giver(
        piece_code_input, INPUT_BOARD_MATRIX).copy()
    output = simple_possible_destination_giver(
        piece_code_input, INPUT_BOARD_MATRIX)
    ...
    for destination in initially_allowed_boxes:
        ...
        if TEMP_BOARD_MATRIX[destination // 8][destination % 8] in TEMP_PIECES_ALIVE[int(not which_side_move())]:
            TEMP_PIECES_ALIVE[int(not which_side_move())].remove(
        ...
        for opp_piece in TEMP_PIECES_ALIVE[int(not which_side_move())]:
            if piececode_to_board_matrix_index_converter(king_code(which_side_move()),
    ...
    for i in range(2):
        ...
        for opp_piece in PIECES_ALIVE[not i]:
            if pieceCodeToPositionConverterMainBoard(king_code(i)) in simple_possible_destination_giver(opp_piece):
                ...
                break
        if truth_value == 0:
            widget_colourer_and_bg_colour_attribute_setter(piecesList[king_code(
                i)], boxColourList[boxesList[pieceCodeToPositionConverterMainBoard(king_code(i))].colour])
...
#     computer_piece_mover(move[0], move[1])
